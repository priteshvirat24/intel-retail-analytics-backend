...
class ShareOfShelfEngine:
    ...
    def compute_share_of_shelf(cls, products: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        intel_skus = [p for p in products if p["is_intel_cpu"]]
        amd_skus = [p for p in products if p["processor_brand"] == "AMD"]
        apple_skus = [p for p in products if p["processor_brand"] == "Apple"]
        qualcomm_skus = [p for p in products if p["processor_brand"] == "Qualcomm"]

        overall_sos = {
            "total_skus": total_skus,
            "intel_skus_count": len(intel_skus),
            "intel_sos_pct": round((len(intel_skus) / total_skus * 100), 1),
            "amd_skus_count": len(amd_skus),
            "amd_sos_pct": round((len(amd_skus) / total_skus * 100), 1),
            "apple_skus_count": len(apple_skus),
            "apple_sos_pct": round((len(apple_skus) / total_skus * 100), 1),
            "qualcomm_skus_count": len(qualcomm_skus),
            "qualcomm_sos_pct": round((len(qualcomm_skus) / total_skus * 100), 1),
        }
        ...
        for ret in retailers:
            ret_items = [p for p in products if p["retailer"] == ret]
            r_total = len(ret_items)
            r_intel = sum(1 for p in ret_items if p["is_intel_cpu"])
            r_amd = sum(1 for p in ret_items if p["processor_brand"] == "AMD")
            r_apple = sum(1 for p in ret_items if p["processor_brand"] == "Apple")
            r_qualcomm = sum(1 for p in ret_items if p["processor_brand"] == "Qualcomm")
        ...
        oems = sorted(list(set(p["oem"] for p in products)))
        oem_sos_list = []
        for oem in oems:
            oem_items = [p for p in products if p["oem"] == oem]
            o_total = len(oem_items)
            o_intel = sum(1 for p in oem_items if p["is_intel_cpu"])
            o_amd = sum(1 for p in oem_items if p["processor_brand"] == "AMD")
            o_apple = sum(1 for p in oem_items if p["processor_brand"] == "Apple")
            o_qualcomm = sum(1 for p in oem_items if p["processor_brand"] == "Qualcomm")
