...
# TikTok data delivery to Amazon S3

> Configure the Bright Data TikTok Scraper API to deliver scraped profiles, posts, comments and shop data directly to your Amazon S3 bucket on job completion.

This guide shows you how to configure the Bright Data TikTok Scraper API to deliver scraped data directly to your Amazon S3 bucket when a collection job completes.
...
2. Enter a bucket name (e.g., `tiktok-scraper-data`)
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::tiktok-scraper-data/*"
    }
  ]
}
```

Replace `tiktok-scraper-data` with your actual bucket name.
...
4. Enter your credentials:
   ...
   * **Path prefix** (optional): A folder path within the bucket (e.g., `tiktok/profiles/`)
...
```bash theme={null}
aws s3 ls s3://tiktok-scraper-data/tiktok/profiles/
```
...
```bash theme={null}
aws s3 cp s3://tiktok-scraper-data/tiktok/profiles/s_m1a2b3c4d5e6f7g8h.json ./results.json
