...
# Async TikTok data collection

> Trigger large-scale TikTok scraping jobs, monitor progress, and retrieve results using the Bright Data async /trigger endpoint. Supports batches of 20 URLs.

This guide shows you how to scrape TikTok data at scale using the asynchronous `/trigger` endpoint. Use this when you have more than 20 URLs, need discovery, or want delivery to a webhook or S3.
...
* Familiarity with the [synchronous request flow](/datasets/scrapers/tiktok/send-first-request)
...
<CodeGroup>
  ...
  curl -X POST \
    "https://api.brightdata.com/datasets/v3/trigger?dataset_id=gd_l1villgoiiidt09ci&format=json" \
...
You've successfully triggered, monitored, and downloaded a batch TikTok scraping job.
...
```bash theme={null}
curl -X POST \
  "https://api.brightdata.com/datasets/v3/trigger?dataset_id=gd_l1villgoiiidt09ci&format=json&endpoint=https://your-server.com/webhook" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '[{"url": "https://www.tiktok.com/@tiktok"}]'
```

See [How to receive TikTok data via webhooks](/datasets/scrapers/tiktok/data-delivery/webhooks) for the full setup.
...
<Accordion title="Snapshot status shows 'failed'?">
  Check that all input URLs are valid TikTok URLs. Review the error details in the snapshot response or in the [Logs tab](https://brightdata.com/cp/scrapers) of your Bright Data dashboard.
</Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Deliver to S3" icon="bucket" href="/datasets/scrapers/tiktok/data-delivery/amazon-s3">
