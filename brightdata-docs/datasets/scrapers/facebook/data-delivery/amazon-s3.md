...
# Facebook data delivery to Amazon S3

> Configure the Bright Data Facebook Scraper API to deliver scraped profiles, posts and marketplace data directly to your Amazon S3 bucket on job completion.

This guide shows you how to configure the Bright Data Facebook Scraper API to deliver scraped data directly to your Amazon S3 bucket when a collection job completes.
...
2. Enter a bucket name (e.g., `facebook-scraper-data`)
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::facebook-scraper-data/*"
    }
  ]
}
```

Replace `facebook-scraper-data` with your actual bucket name.
...
4. Enter your credentials:
   ...
   * **Path prefix** (optional): A folder path within the bucket (e.g., `facebook/profiles/`)
...
```bash theme={null}
aws s3 ls s3://facebook-scraper-data/facebook/profiles/
```
...
```bash theme={null}
aws s3 cp s3://facebook-scraper-data/facebook/profiles/s_m1a2b3c4d5e6f7g8h.json ./results.json
