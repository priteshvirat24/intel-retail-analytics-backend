...
# How to scrape Google data in bulk

> Trigger large-scale Google scraping jobs, monitor progress and retrieve results using the Bright Data async /trigger endpoint. Returns up to 1 GB per response.
...
* Familiarity with the [synchronous request flow](/datasets/scrapers/google/send-first-request)
...
You've successfully triggered, monitored and downloaded a batch Google scraping job. See the [Download Snapshot API reference](/api-reference/scrapers/delivery-apis/download-snapshot) for format options and part-by-part download flow.
...
See [Webhook delivery](/datasets/scrapers/google/data-delivery/webhooks) for the full setup.
...
<Accordion title="Snapshot status shows 'failed'?">
  Check that all input URLs are valid and correctly formatted for the target Google product. Review the error details in the snapshot response or in the [Logs tab](https://brightdata.com/cp/scrapers) of your Bright Data dashboard.
</Accordion>

<Accordion title="Results are incomplete or missing some records?">
...
<CardGroup cols={2}>
  <Card title="Webhook delivery" icon="webhook" href="/datasets/scrapers/google/data-delivery/webhooks">
