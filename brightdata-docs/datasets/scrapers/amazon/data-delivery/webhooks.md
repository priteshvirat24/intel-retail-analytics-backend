...
# Amazon data delivery via webhook

> Configure the Bright Data Amazon Scraper API to push scraped data to your HTTPS webhook as each collection job completes across 5 supported endpoints.

This guide shows you how to set up webhook delivery so your server receives scraped Amazon data automatically when a collection job finishes.

## Prerequisites

* A [Bright Data account](https://brightdata.com/cp/start) with an active API key
* Familiarity with the [async request workflow](/datasets/scrapers/amazon/async-requests)
...
Once the collection completes (typically 30-60 seconds for a few products), check your webhook.site page. You should see a `POST` request with the scraped data.
...
<Accordion title="Payload too large for your server?">
  Large collections can produce payloads up to 1 GB. Set `express.json({ limit: "100mb" })` in Express.js or equivalent in your framework. If you need to handle very large datasets, use [S3 delivery](/datasets/scrapers/amazon/data-delivery/amazon-s3) instead.
</Accordion>

## Next steps

<CardGroup cols={2}>
  <Card title="Deliver to Amazon S3" icon="bucket" href="/datasets/scrapers/amazon/data-delivery/amazon-s3">
