...
# YouTube data delivery via webhook

> Configure the Bright Data YouTube Scraper API to push scraped channel, video and comment data to your HTTPS webhook across 3 endpoints as each job completes.

This guide shows you how to set up webhook delivery so your server receives scraped YouTube data automatically when a collection job finishes.

## Prerequisites

* A [Bright Data account](https://brightdata.com/cp/start) with an active API key
* Familiarity with the [async request workflow](/datasets/scrapers/youtube/async-requests)
...
<CodeGroup>
  ...
  curl -X POST \
    "https://api.brightdata.com/datasets/v3/trigger?dataset_id=gd_lk538t2k2p1k3oos71&format=json&endpoint=https://webhook.site/abc-123-def&uncompressed_webhook=true" \
  ...
  const response = await fetch(
    `https://api.brightdata.com/datasets/v3/trigger?dataset_id=gd_lk538t2k2p1k3oos71&format=json&endpoint=${encodeURIComponent(WEBHOOK_URL)}&uncompressed_webhook=true`,
    {
      method: "POST",
      headers: {
        "Authorization": "Bearer YOUR_API_KEY",
...
Once the collection completes (typically 30-60 seconds for a few channels), check your webhook.site page. You should see a `POST` request with the scraped data.
...
app.post("/webhook/youtube", (req, res) => {
  ...
  for (const channel of channels) {
    console.log(`- ${channel.channel_name} (${channel.subscribers} subscribers)`);
  }
...
def handle_webhook():
    ...
    for channel in channels:
        print(f"- {channel['channel_name']} ({channel.get('subscribers')} subscribers)")
...
```bash theme={null}
curl -X POST \
  "https://api.brightdata.com/datasets/v3/trigger?dataset_id=gd_lk538t2k2p1k3oos71&format=json&endpoint=https://your-server.com/webhook&webhook_header_Authorization=Bearer+YOUR_SECRET_TOKEN" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
...
<Accordion title="Payload too large for your server?">
  Large collections can produce payloads up to 1 GB. Set `express.json({ limit: "100mb" })` in Express.js or equivalent in your framework. If you need to handle very large datasets, use [S3 delivery](/datasets/scrapers/youtube/data-delivery/amazon-s3) instead.
</Accordion>

## Next steps

<CardGroup cols={2}>
  <Card title="Deliver to Amazon S3" icon="bucket" href="/datasets/scrapers/youtube/data-delivery/amazon-s3">
