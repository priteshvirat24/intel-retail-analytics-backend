...
# X (Twitter) data delivery to Amazon S3

> Configure the Bright Data X (Twitter) Scraper API to deliver scraped profiles, posts and search results directly to your Amazon S3 bucket on job completion.

This guide shows you how to configure the Bright Data X Scraper API to deliver scraped data directly to your Amazon S3 bucket when a collection job completes.
...
2. Enter a bucket name (e.g., `twitter-scraper-data`)
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::twitter-scraper-data/*"
    }
  ]
}
```

Replace `twitter-scraper-data` with your actual bucket name.
...
4. Enter your credentials:
   ...
   * **Path prefix** (optional): A folder path within the bucket (e.g., `twitter/profiles/`)
...
```bash theme={null}
aws s3 ls s3://twitter-scraper-data/twitter/profiles/
```
...
```bash theme={null}
aws s3 cp s3://twitter-scraper-data/twitter/profiles/s_m1a2b3c4d5e6f7g8h.json ./results.json
