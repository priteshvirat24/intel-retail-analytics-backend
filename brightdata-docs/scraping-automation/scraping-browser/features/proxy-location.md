...
# Browser API geolocation targeting

> Geo-target the Bright Data Browser API to countries, states or cities across 195 countries. Automatic peer selection is on by default; manual overrides.
...
* [**Country**](/scraping-automation/scraping-browser/features/proxy-location#how-to-target-a-country) - targets peers from a specific country
* [**Geolocation radius**](/scraping-automation/scraping-browser/features/proxy-location#how-to-set-the-geolocation-radius) – targets peers using an exact latitude, longitude, and radius simulating a precise physical location for more granular geo-targeted data collection.
...
<ParamField path="distance" type="number">
  The radius of the geolocation in kilometers. Defines the maximum distance from the provided latitude and longitude within which to search for available peers.
</ParamField>

<ParamField path="strict" default="true" type="boolean">
  Defines the desired behavior in case no peers are available in the specified distance.

  `strict: true` - Bright Data will search for available peers only within the specific distance.
...
### How to use proxy location targeting
...
## How to get proxy geolocation

Use this command to retrieve the exact geo-location information of the proxy peer assigned to your current browser session. This is useful for logging and verifying the assigned peer's location on your end.

### Proxy geolocation CDP command
