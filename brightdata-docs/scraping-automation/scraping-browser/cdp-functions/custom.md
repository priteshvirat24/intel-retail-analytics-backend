...
> Reference for Bright Data Browser API custom CDP functions that extend the standard Chrome DevTools Protocol with unblocking and session controls.
...
<AccordionGroup>
  <Accordion title="Captcha.solve">
    ...
    **Examples**
    ...
    <Note>
      If CAPTCHA-solving fails, please attempt a retry. If the issue persists, submit a support request detailing the specific problem you encountered.
    </Note>
  </Accordion>
  ...
  <Accordion title="Captcha.setAutoSolve">
    ...
    <CodeGroup>
      ...
      CaptchaOptions : {
        ...
        ... // additional options related to captcha type
      }
      ```
    </CodeGroup>
...
Enabling the `AdBlock` feature can reduce **bandwidth usage** and **improve performance** on ad-heavy websites by blocking ad and tracker requests before they are downloaded.

### Ad blocker CDP commands
...
| --------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `version` | `string`   | Blocking engine version. Pass `'2'` (recommended) to use the current engine with up-to-date filter lists. When omitted, a legacy engine is used.                                                                                                                                    |
| `mode`    | `string`   | Optional, requires `version: '2'`. Blocking coverage: `'ads'` (the default) blocks ad networks; `'full'` also blocks trackers and analytics - the widest coverage and biggest bandwidth reduction; `'none'` disables the built-in lists, so only the domains in `list` are blocked. |
| `list`    | `string[]` | Optional, requires `version: '2'`. Custom domains to block **in addition** to the `mode` coverage. Each entry blocks requests to that domain and all of its subdomains.                                                                                                             |

<Tip>
  Enable ad blocking before navigating to the target page.
</Tip>
...
// widest coverage (ads + trackers):
// await client.send('Unblocker.enableAdBlock', { version: '2', mode: 'full' });

// additionally block custom domains:
...
<Note>
  Blocked requests are reported in the browser's DevTools console with the message `Resource was blocked by ad-blocking`.
</Note>
...
## How to block cookie consent banners

The `CookieBlock` feature blocks the scripts of common third-party consent-management platforms (CMPs), so cookie consent banners never load - no need to find and click "Accept" buttons in your automation.

### Cookie blocker CDP commands

* `Unblocker.enableCookieBlock` – Enables cookie consent blocking (default: off)
* `Unblocker.disableCookieBlock` – Disables cookie consent blocking

#### Parameters

| Parameter | Type       | Description                                                                                                                                          |
| --------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list`    | `string[]` | Optional. Custom domains to block **in addition** to the built-in CMP coverage. Each entry blocks requests to that domain and all of its subdomains. |

<Tip>
  Enable cookie blocking before navigating to the target page.
</Tip>
...
// additionally block a site-specific consent script:
...
<Note>
  The built-in coverage blocks banners served by common third-party CMPs. Banners hosted by the website itself are not covered by it - to block those, find the domain serving the consent script (for example, in the DevTools Network tab) and pass it in the `list` parameter, as in the example above.
</Note>
...
### Session persistence CDP commands
...
Use this command to retrieve the unique ID of your current browser session. This is useful when you need to look up session logs via the [Session Logs API](/api-reference/browser-api/get-session), such as investigating errors, unexpected behavior, or high bandwidth usage.

### Session ID CDP command
...
### Faster text input CDP commands
...
<AccordionGroup>
  ...
  <Accordion title="Code examples">
    * Replace the example values `SBR_ZONE_FULL_USERNAME:SBR_ZONE_PASSWORD` with your valid Browser API credentials.
