...
<Steps>
  <Step title="前置条件">
    ...
    <Tip>
      无需用户名。自托管 MCP 服务器仅使用您的 `API_TOKEN` 进行身份验证。用户名（格式：`brd-customer-{customer_id}-zone-{zone_name}`）用于原生代理访问，而非 MCP。如果第三方工具要求填写用户名，请将其留空。
    </Tip>
  </Step>
