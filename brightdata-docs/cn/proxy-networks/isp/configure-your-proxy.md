...
> 了解如何有效地配置您的 ISP 代理设置、选择 IP 类型、启用缓存代理并确保 100% 正常运行时间以实现最佳性能。

要访问您的代理配置，请在 **“配置”** 选项卡中打开您的区域。

## IP 类型

Bright Data 提供 3 种类型的 ISP 代理：
...
默认情况下，端口 `80` 和 `443` 可用，支持 HTTP 和 HTTPS 及 SOCKS5 协议。我们的 ISP 代理网络还支持所有高于 `1024` 的端口。 [了解有关端口和协议的更多信息...](/cn/proxy-networks/faqs#如何查看支持的端口和协议)
