...
<Note>
  结果数据在采集完成后可供下载 16 天。为避免过期，请务必在 16 天内下载数据，或配置交付方式以便自动将数据发送至您的存储。
</Note>

<Note>
  如有需要，结果可以拆分为多个部分：

  1. 通过指定 `batch_size` 和 `part=1` 下载第一部分
  2. 使用 <a href="/cn/api-reference/scrapers/management-apis/get-snapshot-delivery-parts#获取-snapshot-传输部分" target="_blank">此端点</a> 查看已生成的部分数量
  3. 修改 `part` 参数下载剩余部分。（`batch_size`、`format` 和 `compress` 在每次调用中都必须保持一致）
</Note>


## OpenAPI

````yaml cn-dca-api GET /datasets/v3/snapshot/{snapshot_id}
openapi: 3.1.0
info:
  ...
  /datasets/v3/snapshot/{snapshot_id}:
    get:
      ...
      parameters:
        ...
        - name: part
          in: query
          description: 如果提供了 batch_size，则指定下载哪一部分
          required: false
          schema:
            type: integer
          example: 1
